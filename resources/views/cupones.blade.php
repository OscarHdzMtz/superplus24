    ...
    <div class="row_cards_promo">
        ...
        <div class="col-md-3 col-sm-6 mb-3">
            <div data-aos="zoom-in" class="{{-- single-contentpromo --}} clic_abre_modal"> {{-- la parte comentada borde la tarjeta y le pone sombra --}}
                <img id="get_image_promo" class="popou_img_promo"src="{{asset('/img/ofertas/'.$cupon->image)}}" alt="{{$cupon->image}}">
                ...
                {{-- <form action="{{ route('cupones.store', $oferta->id) }}" method="POST">
                    @csrf
                ...
                {{ Form::token() }}
                    {{-- <div class="modal-footer">   --}}                      
                        <button type="submit" class="btn btn-primary btn-block">GENERAR CUPON</button>
                    {{-- </div> --}}
                {!! Form::close() !!}
            </div>
        </div>
